    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <template v-else>
        ...
        <FilterBar
          ...
          v-model:selectedCountry="selectedCountryQuery"
        ...
        <div class="w-full flex flex-col lg:flex-row gap-6 mt-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 flex-[4]">
            ...
            <p v-else class="text-center col-span-full text-gray-400 text-lg">Không có dữ liệu phim cho quốc gia này.</p>
          </div>
...
// Thêm biến reactive cho selectedCountryQuery để đồng bộ với v-model của FilterBar
const selectedCountryQuery = ref(route.query.country || ''); // country từ URL query
...
const countryName = computed(() => {
  if (countryStore.getAllCountries.length > 0 && selectedCountryQuery.value) {
    const country = countryStore.getAllCountries.find(c => c.slug === selectedCountryQuery.value);
    return country ? country.name : null;
  }
  // Nếu không có selectedCountryQuery.value nhưng có countrySlug (tức là lọc theo path), sử dụng countrySlug
  if (countryStore.getAllCountries.length > 0 && countrySlug.value) {
    const country = countryStore.getAllCountries.find(c => c.slug === countrySlug.value);
    return country ? country.name : null;
  }
  return null;
});
...
const categoryName = computed(() => {
  if (categoryStore.getAllCategories.length > 0 && selectedCategory.value) {
    const category = categoryStore.getAllCategories.find(cat => cat.slug === selectedCategory.value);
...
const fetchMovies = async () => {
  ...
  console.log('ListBaseCountry: Bắt đầu fetchMovies.');
  console.log('ListBaseCountry: Selected Category Slug (query):', selectedCategory.value);
  console.log('ListBaseCountry: Selected Country Slug (query):', selectedCountryQuery.value);
  console.log('ListBaseCountry: Selected Year (query):', selectedYear.value);
  console.log('ListBaseCountry: Route Country Slug (path):', countrySlug.value);
  console.log('ListBaseCountry: Current Page:', currentPage.value);
  ...
  try {
    ...
    if (currentCountryFilterName) {
      console.log('ListBaseCountry: Áp dụng lọc theo Quốc gia:', currentCountryFilterName);
      queryBuilder = queryBuilder.contains('country', [currentCountryFilterName]);
    } else if ((countrySlug.value || selectedCountryQuery.value) && countryStore.getAllCountries.length > 0) {
      console.warn(`ListBaseCountry: Không tìm thấy tên quốc gia cho slug: '${countrySlug.value || selectedCountryQuery.value}'. Không áp dụng lọc theo quốc gia.`);
    } else {
      console.log('ListBaseCountry: Không có slug quốc gia hợp lệ hoặc dữ liệu quốc gia chưa tải.');
    }

    // Lọc theo Thể loại:
    const currentCategoryFilterName = categoryName.value;
    if (currentCategoryFilterName) {
      console.log('ListBaseCountry: Áp dụng lọc theo Thể loại:', currentCategoryFilterName);
      queryBuilder = queryBuilder.contains('genres', [currentCategoryFilterName]);
    } else if (selectedCategory.value && categoryStore.getAllCategories.length > 0) {
      console.warn(`ListBaseCountry: Không tìm thấy tên thể loại cho slug: '${selectedCategory.value}'. Không áp dụng lọc theo thể loại.`);
    } else {
      console.log('ListBaseCountry: Không có slug thể loại hợp lệ hoặc dữ liệu thể loại chưa tải.');
    }
    ...
    if (selectedYear.value) {
      ...
      if (!isNaN(year)) {
        console.log('ListBaseCountry: Áp dụng lọc theo Năm:', year);
        queryBuilder = queryBuilder.eq('year', year);
      }
    } else {
      console.log('ListBaseCountry: Không có năm được chọn.');
    }
    ...
    movies.value = data || [];
    totalPage.value = Math.ceil((count || 0) / MOVIES_PER_PAGE);

    console.log("ListBaseCountry: Phim đã tải từ Supabase:", movies.value);
    console.log("ListBaseCountry: Tổng số lượng phim:", count, "Tổng số trang:", totalPage.value);

  } catch (err) {
    console.error('Lỗi khi fetch phim từ Supabase (ListBaseCountry):', err);
    error.value = 'Không thể tải dữ liệu phim. Vui lòng thử lại sau.';
    movies.value = [];
    totalPage.value = 1;
  } finally {
    loading.value = false;
    console.log('ListBaseCountry: Kết thúc fetchMovies. Loading:', loading.value, 'Error:', error.value);
  }
};

// Hàm xử lý khi người dùng đổi trang (từ pagination)
const handlePageChange = (page) => {
  if (page >= 1 && page <= totalPage.value) {
    // Debugging current countrySlug before path construction
    console.log('DEBUG handlePageChange: countrySlug.value before path construction:', countrySlug.value);

    // Ensure currentPathSegment avoids double slashes
    let currentPathSegment;
    if (countrySlug.value) {
      currentPathSegment = `country/${encodeURIComponent(countrySlug.value)}`;
    } else {
      currentPathSegment = 'country';
    }

    // Construct the query parameters, removing undefined ones
    const newQuery = {};
    if (selectedCategory.value) {
      newQuery.category = encodeURIComponent(selectedCategory.value);
    }
    if (selectedCountryQuery.value) {
      newQuery.country = encodeURIComponent(selectedCountryQuery.value);
    }
    if (selectedYear.value) {
      newQuery.year = encodeURIComponent(selectedYear.value);
    }

    const targetPath = `/${currentPathSegment}/page/${page}`;
    console.log('DEBUG handlePageChange: Target path being pushed:', targetPath);
    console.log('DEBUG handlePageChange: Query parameters being pushed:', newQuery);
...
onMounted(async () => {
  console.log('ListBaseCountry: Component mounted.');

  // Lấy phim đề cử cho sidebar (nếu chưa có)
  if (recommendedMoviesStore.getRecommended.length === 0) {
    console.log('ListBaseCountry: Fetching recommended movies for sidebar...');
    await recommendedMoviesStore.getRecommendedMovies();
  } else {
    console.log('ListBaseCountry: Recommended movies already in store. Count:', recommendedMoviesStore.getRecommended.length);
  }

  // Lấy danh sách thể loại và quốc gia (nếu chưa có)
  if (categoryStore.getAllCategories.length === 0) {
    console.log('ListBaseCountry: Fetching categories...');
    await categoryStore.getCategory();
    console.log('ListBaseCountry: Categories loaded. Count:', categoryStore.getAllCategories.length, 'Data:', categoryStore.getAllCategories);
  } else {
    console.log('ListBaseCountry: Categories already in store. Count:', categoryStore.getAllCategories.length, 'Data:', categoryStore.getAllCategories);
  }

  if (countryStore.getAllCountries.length === 0) {
    console.log('ListBaseCountry: Fetching countries...');
    await countryStore.getCountry();
    console.log('ListBaseCountry: Countries loaded. Count:', countryStore.getAllCountries.length, 'Data:', countryStore.getAllCountries);
  } else {
    console.log('ListBaseCountry: Countries already in store. Count:', countryStore.getAllCountries.length, 'Data:', countryStore.getAllCountries);
  }
  console.log('ListBaseCountry: Available years computed:', availableYears.value);
  ...
  watch(
    ...
    (newParams) => {
      ...
      console.log('ListBaseCountry: route.params changed. Triggering fetchMovies (if stores ready).');
      // Fetch phim chỉ khi category và country stores đã load xong
      // hoặc đảm bảo `categoryName` và `countryName` có giá trị sau khi store cập nhật
  ...
  watch(
    ...
    (newQuery) => {
      ...
      selectedCountryQuery.value = newQuery.country || ''; // Cập nhật ref này
      selectedYear.value = newQuery.year || '';
      console.log('ListBaseCountry: route.query changed. Triggering fetchMovies (if stores ready).');
      // Fetch phim chỉ khi category và country stores đã load xong
      if (categoryStore.getAllCategories.length > 0 && countryStore.getAllCountries.length > 0) {
        fetchMovies();
      }
  ...
  watch([
    ...
    if ((newCategoryCount > 0 && newCountryCount > 0) && (!newCategoryLoading && !newCountryLoading)) {
      if (!loading.value) { // Tránh gọi quá nhiều lần nếu đang loading chính
          console.log('ListBaseCountry: Category or Country store data loaded and not loading. Triggering fetchMovies.');
          fetchMovies();
      }
    }
  });
  ...
  if (categoryStore.getAllCategories.length > 0 && countryStore.getAllCountries.length > 0) {
    console.log('ListBaseCountry: Stores already have data, calling fetchMovies for initial load.');
    fetchMovies();
  } else {
    console.log('ListBaseCountry: Stores empty on initial mount, waiting for watchers.');
  }
});
</script>
